...
group = "tail";
const tail =
  "M 710 844 C 756 802 802 799 845 819 Q 870 815 881 796 C 894 818 890 839 879 853 Q 906 856 926 840 C 927 864 910 881 890 892 Q 864 913 827 910 C 778 910 739 886 710 844 Z";
p(tail, grad(760, 810, 865, 910, ["#f0d9b9", "#e8ceb0", "#cda595"]), ink, 5);
p(
  "M 745 864 Q 812 901 865 887 L 851 879 Q 894 884 923 852 C 911 890 880 911 827 910 Q 775 906 745 864 Z",
  "#b99289",
  "#00000000",
  0,
  0.45,
...
p(
  "M 753 837 Q 791 813 840 833 L 862 830 Q 829 850 799 842 Q 779 839 753 837 Z",
  "#f8e2bd",
  "#00000000",
  0,
  0.75,
...
p("M 816 901 Q 805 896 800 890 M 865 887 Q 857 885 851 879", undefined, "#ad827c", 2, 0.45);
group = "hair-back";
...
p(
  "M 510 847 L 591 854 L 589 970 L 511 970 Z",
  grad(513, 864, 574, 940, ["#fff0e7", "#f4d5cf"]),
  ink,
  4.5,
);
const sockL =
  "M 508 942 Q 520 933 534 945 Q 545 930 559 944 Q 573 931 590 942 L 590 1067 Q 589 1093 565 1098 L 530 1097 Q 511 1093 510 1074 Z";
...
layer += 3; // The video shows three broad yellow bands; preserve following IDs.
p(
...
p(
  "M 623 835 L 708 839 L 708 950 L 625 950 Z",
  grad(623, 848, 706, 916, ["#fff0e7", "#f0cfc8"]),
  ink,
  4.5,
);
const sockR =
  "M 622 923 Q 638 912 649 926 Q 663 912 676 925 Q 693 912 708 925 L 710 1057 Q 706 1080 684 1085 L 649 1085 Q 626 1080 625 1060 Z";
...
const dressLayer = layer;
layer = 2300;
p("M 550 649 L 560 653 L 543 714 L 532 708 Z", "#ffe6a5", ink, 2.8);
p("M 670 649 L 681 655 L 628 722 L 615 713 Z", "#f9d993", ink, 2.8);
const dressBase = shapes.find((shape) => shape.group === "dress");
if (dressBase?.layer === undefined) throw new Error("The apron base must precede its straps.");
const strapsLayer = dressBase.layer;
shapes.slice(-2).forEach((shape, index) => {
  shape.layer = strapsLayer + (index + 1) / 10;
});
layer = dressLayer;
group = "neck-bell";
p("M 566 610 L 650 610 L 665 677 Q 617 711 561 676 Z", "#fff0e5", ink, 4);
p("M 545 650 Q 607 640 669 650 L 678 669 Q 612 692 543 672 Z", "#fffaf3", ink, 3.5);
layer += 1; // Crew-neck shirt: no pointed collar in the default costume.
...
layer += 2; // One prominent highlight per iris, as seen in the video.
p(
...
p("M 620 555 Q 628 553 633 558 L 629 571 Q 622 568 620 555 Z", "#fff8ed", ink, 1.6);
p("M 574 562 L 570 554 M 632 558 L 637 550", undefined, ink, 3);
oval(611, 551, 3, 2, "#d8aaa5", "#00000000", 0, 0.3);
group = "hair-front";
const front =
  "M 395 355 C 368 285 414 223 481 206 C 527 185 567 185 601 190 Q 570 165 590 148 Q 611 131 642 145 Q 615 143 615 162 Q 616 177 635 184 C 720 174 789 212 818 283 C 845 349 832 478 800 520 Q 780 543 757 546 C 782 536 775 465 758 421 Q 748 383 724 355 C 741 383 751 401 759 408 Q 723 413 690 393 C 666 404 639 405 615 390 Q 588 405 558 393 Q 534 401 515 385 Q 493 397 478 381 C 457 428 444 469 437 506 Q 416 493 407 480 C 385 452 374 417 385 384 Q 370 390 356 377 Q 385 367 395 355 Z";
...
group = "ears";
const earL =
  "M 467 219 C 421 199 397 214 370 238 C 340 264 310 271 285 294 C 271 314 287 341 313 342 C 349 337 380 309 411 291 C 436 276 455 256 471 241 Q 473 229 467 219 Z";
...
const earR =
  "M 775 222 C 822 207 851 224 874 248 C 904 273 937 273 963 271 C 984 265 984 294 962 320 C 944 346 926 361 905 365 Q 876 370 857 353 C 825 331 807 289 785 268 Q 773 248 775 222 Z";
...
const slingLayer = layer;
layer = 2400;
p("M 347 734 Q 326 718 323 735 Q 324 753 346 746 L 357 738 Z", "#f6d184", ink, 3.5);
p("M 358 733 Q 374 715 381 731 Q 390 749 362 746 Z", "#f6d184", ink, 3.5);
p("M 345 731 Q 354 726 362 734 L 361 748 Q 352 754 345 747 Z", "#ffe1a0", ink, 3);
p("M 329 733 L 340 739 M 365 738 L 376 733", undefined, "#c0905e", 2.2);
p("M 347 734 Q 352 730 357 734", undefined, "#fff0bd", 2);
layer = slingLayer;
group = "grip";
p(
...
for (const shape of shapes) {
  ...
  const matrix: Matrix = shape.transform ?? [1, 0, 0, 1, 0, 0];
  const [a, b] = matrix;
  let [, , c, d, e, f] = matrix;
  // The short head keeps oval eyes; scale iris, lashes and highlights together.
  if (name === "eyes" && (shape.kind !== "curve" || shape.strokeWidth !== 8)) {
    e += c * 480 * (1 - 1.16);
    f += d * 480 * (1 - 1.16);
    c *= 1.16;
    d *= 1.16;
